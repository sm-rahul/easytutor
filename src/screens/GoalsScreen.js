...
import { IconBadge } from '../components/AppIcon';
...
export default function GoalsScreen({ navigation }) {
  ...
  // Overall streak message
  const bothMet = lessonGoalMet && timeGoalMet;
  const oneMet = lessonGoalMet || timeGoalMet;
  ...
  return (
    <View style={s.root}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        {/* Header */}
        <Animated.View style={[s.headerSection, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
          <LinearGradient colors={GRADIENTS.accent} style={s.headerIcon}>
            <Ionicons name="flag" size={rs(28)} color={COLORS.white} />
          </LinearGradient>
          <Text style={s.headerTitle}>My Goals</Text>
          <Text style={s.headerSub}>
            {bothMet ? 'All goals met today!' : oneMet ? 'Keep going, almost there!' : 'Start learning to hit your goals'}
          </Text>
        </Animated.View>
        ...
        <Animated.View style={{ opacity: fadeIn, transform: [{ translateY: slideUp }] }}>
          ...
          <View style={s.summaryCard}>
            <View style={s.summaryRow}>
              <View style={s.summaryItem}>
                <Text style={s.summaryNum}>{goals.todayLessons}</Text>
                <Text style={s.summaryLabel}>Lessons</Text>
              </View>
              <View style={s.summaryDivider} />
              <View style={s.summaryItem}>
                <Text style={s.summaryNum}>{formatTime(goals.todayReadingMinutes)}</Text>
                <Text style={s.summaryLabel}>Reading</Text>
              </View>
              <View style={s.summaryDivider} />
              <View style={s.summaryItem}>
                <Text style={s.summaryNum}>{stats.totalScans || 0}</Text>
                <Text style={s.summaryLabel}>Scans</Text>
              </View>
            </View>
          </View>
          ...
          <View style={s.section}>
            <View style={s.sectionHeader}>
              <Ionicons name="today-outline" size={rs(18)} color={COLORS.accent} />
            ...
            <View style={s.goalCard}>
              ...
              <Text style={s.progressPercent}>{Math.round(lessonProgress * 100)}%</Text>
            </View>
            ...
            <View style={s.goalCard}>
              ...
              <Text style={s.progressPercent}>{Math.round(timeProgress * 100)}%</Text>
            </View>
          ...
          {performance && performance.overall.totalAttempts > 0 && (
            <View style={s.section}>
              <View style={s.sectionHeader}>
                <Ionicons name="school-outline" size={rs(18)} color={COLORS.accent} />
                <Text style={s.secTitle}>Quiz Performance</Text>
              </View>

              <TouchableOpacity activeOpacity={0.7} onPress={() => navigation.navigate('QuizHistory')}>
                ...
                <View style={s.quizStatsHint}>
                  <Text style={s.quizStatsHintText}>Tap to view quiz history</Text>
                  <Ionicons name="chevron-forward" size={rs(12)} color={COLORS.textMuted} />
                </View>
              </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  // Header
  headerSection: { alignItems: 'center', marginBottom: rs(20) },
  headerIcon: {
    width: rs(60), height: rs(60), borderRadius: rs(20),
    justifyContent: 'center', alignItems: 'center', ...SHADOWS.accentGlow,
  },
  headerTitle: { ...FONTS.h1, color: COLORS.textPrimary, marginTop: rs(12) },
  headerSub: { ...FONTS.bodySmall, color: COLORS.textMuted, marginTop: rs(4), textAlign: 'center' },

  // Today's Summary
  summaryCard: {
    backgroundColor: COLORS.bgCard, borderRadius: RADIUS.xl, padding: rs(18),
    borderWidth: 1, borderColor: COLORS.border, marginBottom: rs(20), ...SHADOWS.card,
  },
  summaryRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around' },
  summaryItem: { alignItems: 'center', gap: rs(4) },
  summaryNum: { fontSize: ms(22), fontWeight: '800', color: COLORS.textPrimary },
  summaryLabel: { ...FONTS.caption, color: COLORS.textMuted },
  summaryDivider: { width: 1, height: rs(36), backgroundColor: COLORS.border },
  ...
  secTitle: { ...FONTS.h3, color: COLORS.textPrimary, flex: 1 },

  // Edit Goal Button
  editGoalBtn: {
    flexDirection: 'row', alignItems: 'center', gap: rs(4),
    paddingHorizontal: rs(12), paddingVertical: rs(6), borderRadius: RADIUS.full,
    borderWidth: 1, borderColor: COLORS.borderAccent, backgroundColor: COLORS.accentGlow,
  },
  ...
  goalCardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: rs(10) },
  ...
  progressPercent: { ...FONTS.caption, color: COLORS.textMuted, textAlign: 'right', marginTop: rs(4) },
  ...
  // Quiz Performance
  quizStatsRow: { flexDirection: 'row', gap: rs(8), marginBottom: rs(10) },
  quizStatCard: {
    flex: 1, alignItems: 'center', gap: rs(5),
    backgroundColor: COLORS.bgCard, borderRadius: RADIUS.xl, padding: rs(14),
  ...
  quizStatsHint: {
    ...
    gap: rs(4), marginTop: rs(6), marginBottom: rs(14),
  },
  quizStatsHintText: { ...FONTS.caption, color: COLORS.textMuted, fontSize: ms(10) },
  quizTypeRow: { flexDirection: 'row', gap: rs(8), flexWrap: 'wrap', marginBottom: rs(14) },
  quizTypeChip: {
  ...
  trendHeader: { flexDirection: 'row', alignItems: 'center', gap: rs(6), marginBottom: rs(12) },
  ...
  trendBarFill: { ...StyleSheet.absoluteFillObject, borderRadius: rs(6) },
  trendDot: {
